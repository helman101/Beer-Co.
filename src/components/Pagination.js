...
export const Pagination = () => {
  ...
  const { currentPage, fullList, beersPerPage } = useSelector((state) => state.beers)
  ...
  return (
    <nav className='d-flex pagination align-items-center'>
      <div className="page-nums d-flex">
        { currentPage > 1 && <div className="page-item" onClick={() => dispatch(previousPage())}>{'< Previous'}</div>}
        {pageNumbers.map(number => (
          number <= maxPages && <div onClick={() => dispatch(changePage(number))} key={number} className="page-item">
            {number}
          </div>
        ))}
        {currentPage !== maxPages && <div className="page-item" onClick={() => dispatch(nextPage())}>{'Next >'}</div>}
      </div>
    </nav>
  )
}
